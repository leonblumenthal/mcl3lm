...
get_image_paths(const std::string &directory, const std::string &file_ending, int limit = 0, bool sort = true) {
...
void dump_map(const Map &map, const std::string &path = "../map_dump") {
  ...
  for (const auto &[keyframe_index, keyframe] : map.keyframes) {
    ...
    for (int i = 0; i < keyframe.keypoints.size(); ++i) {
      out << keyframe.keypoints[i].x() << " " << keyframe.keypoints[i].y() << " " << keyframe.descriptors[i] << std::endl;
    }
  }
